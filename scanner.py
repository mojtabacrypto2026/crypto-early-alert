...
# Confirmed alert management. Prevent stale alert-state entries from
# suppressing a genuinely new signal forever.
...
def smart_alert(result, alert_state, alerts_enabled):
    ...
    # Same confirmed signal is not repeated during the cooldown
    # unless the score strengthens by at least 5 points.
    old_timestamp = int(safe_float(old.get("timestamp"), 0.0))
    within_cooldown = (
        old_timestamp > 0
        and int(time.time()) - old_timestamp < CONFIRMED_ALERT_COOLDOWN_SECONDS
    )
...
# ============================================================
# ALERT STATE MAINTENANCE
# ============================================================

def prune_alert_state(alert_state, now):
    ...
    for symbol, entry in alert_state.items():
        ...
        timestamps = [
            int(safe_float(entry.get("timestamp"), 0.0)),
            int(safe_float(entry.get("fast_timestamp"), 0.0)),
        ]
        ...
        if now - latest <= ALERT_STATE_TTL_SECONDS:
...
def run_scan():
    ...
    alert_state = prune_alert_state(alert_state, now)
