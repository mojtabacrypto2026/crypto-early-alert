...
def get(u):
 r=urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'Crypto-Early-Alert/3.0'}),timeout=20); return json.loads(r.read().decode())
def candles(p,g,h):
 n=int(time.time()); q=urllib.parse.urlencode({'start':n-h*3600,'end':n,'granularity':g}); a=get(f'https://api.coinbase.com/api/v3/brokerage/market/products/{p}/candles?{q}').get('candles',[]); a.sort(key=lambda x:int(x['start'])); return a[:-1]
def avg(a): return sum(a)/len(a) if a else 0
def pct(a,b): return (a-b)/b*100 if b else 0

def scan(p):
 c5=candles(p,'FIVE_MINUTE',30); c15=candles(p,'FIFTEEN_MINUTE',90); c1=candles(p,'ONE_HOUR',180)
...
def send(t):
 ...
 u=f'https://api.telegram.org/bot{BOT_TOKEN}/sendMessage'; d=urllib.parse.urlencode({'chat_id':CHAT_ID,'text':t}).encode(); return json.loads(urllib.request.urlopen(urllib.request.Request(u,data=d),timeout=20).read().decode()).get('ok',False)
